...
OUTPUT_CSV = os.path.join(BASE_DIR, 'pcen_analysis_report.csv')
SAMPLE_RATE = 16000
WINDOW_SIZE_MS = 30
WINDOW_STEP_MS = 10
N_MELS = 64
...
def analyze_file(file_path, label):
    try:
        ...
        stats = {'class_label': label, 'filename': os.path.basename(file_path), 'duration_sec': duration, 'sample_rate': sr, 'mean_frame_energy': mean_energy, 'std_frame_energy': std_energy, 'median_frame_energy': median_energy, 'max_frame_energy': max_energy, 'min_frame_energy': min_energy, 'frame_energy_cv': energy_cv, 'avg_active_ratio': active_ratio, 'avg_silence_ratio': silence_ratio, 'mean_rise_time': mean_rise, 'mean_decay_time': mean_decay, 'mean_energy_ratio_h_l': ratio_h_l, 'spectral_brightness': brightness, 'avg_rise_rate_all_bands': np.mean(rise_rates), 'avg_fall_rate_all_bands': np.mean(fall_rates), 'avg_smoothness_all_bands': np.mean(smoothness)}
...
def analyze():
    print('\n' + '=' * 60)
    print('      SILICON-20: PCEN DATASET ANALYZER')
    print('=' * 60)
    results = []
    for label in CLASSES:
        ...
        if not os.path.exists(class_dir):
            continue
        files = [os.path.join(class_dir, f) for f in os.listdir(class_dir) if f.lower().endswith(('.wav', '.ogg', '.mp3'))]
    ...
    if results:
        ...
        summary = df.groupby('class_label')[['mean_frame_energy', 'frame_energy_cv', 'spectral_brightness', 'mean_rise_time']].mean()
        print(summary)
    else:
